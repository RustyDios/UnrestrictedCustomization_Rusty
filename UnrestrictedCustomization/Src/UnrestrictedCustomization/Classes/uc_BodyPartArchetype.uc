...
public function string GetLabel( bool _genderSuffix ) 
{
	...
	if( _genderSuffix 	&& Template.DlcName != `ModPackage )// do not add prefix for "none"
	{
		if( Template.Genders.Find(eGender_Male) >= 0 )
		{
			 _s @= `Localize("MaleSuffix");
		}
		else if ( Template.Genders.Find(eGender_Female) >= 0 ) 
		{
			_s @= `Localize("FemaleSuffix");
		}
	}
...
protected static function uc_BodyPartArchetype BodyPartArchetype(	class<uc_BodyPartArchetype> _class, string _id) 
{
...
public function AddPartType( uc_EBodyPartType _value ) 
{
...
public function AddGender( EGender _value ) 
{
...
public function AddArmorType( uc_EArmorType _value ) 
{
...
public function AddTechLevel( uc_ETechLevel _value ) 
{
...
public function AddStyle( uc_EStyle _value ) 
{
...
public function SetTemplate( X2BodyPartTemplate _vanillaTemplate ) 
{
	...
	//&& string(_vanillaTemplate.DLCName) == Template.DlcName	//&& _vanillaTemplatePartType == Template.PartType
...
///
public function bool Update(string _displayName, array<EGender> _genders, array<uc_EArmorType> _armorTypes,	array<uc_ETechLevel> _techLevels, array<uc_EStyle> _styles,	string _dlcName = "", string _icon = "") 
{
	...
	if( _displayName == "" ) 
	{
		_displayName = _vanillaDisplayName;
	}
